...
import { Package, Eye, X, Edit, Clock, CheckCircle, MessageCircle, Mail, CreditCard, XCircle, AlertCircle, MoreHorizontal, User, Building2, MapPin, FileText, Calendar, Ban } from 'lucide-react';
...
class PaymentTrackingService {
  ...
  async buildDuePaymentData(paymentMap) {
    ...
    // Filter out payments that are rejected by accounts department
    const nonRejectedPayments = paymentTrackingData.filter(item => {
      // Check if any payment has rejected status
      const hasRejectedPayment = item?.paymentsData?.some(p => {
        const approvalStatus = (p.approval_status || p.status || '').toLowerCase();
        return approvalStatus === 'rejected';
      });
      // Exclude items with rejected payments
      return !hasRejectedPayment;
    });

    const duePayments = nonRejectedPayments.filter(item => 
      item.remainingAmount > 0 || item.paymentStatus === 'advance' || item.paymentStatus === 'due'
    );

    return duePayments.sort((a, b) => {
                        ...
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-3 sm:px-6 py-3 sm:py-4 text-center">
                            <div className="flex items-center justify-end space-x-2">
                              ...
                              <Tooltip text="Amend PI (Cancel products)">
                                <button 
                                  onClick={(e) => {
                                    e.stopPropagation();
