    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-1 text-xl font-bold text-slate-800">{Number(daysLeftInPeriod)}</div>
        </div>
      </div>
      <div className="mt-6">
...
export default function TargetSummary({ isSalesperson, targetSummary, targetList }) {
  if (isSalesperson) {
    return (
      <div className="p-6">
        <SummaryCardsAndProgress targetSummary={targetSummary} />
      </div>
    );
  }
        ...
        <table className="min-w-[600px] w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-600 tracking-wider">Required/day</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-600 tracking-wider">Days left</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 bg-white">
            {list.length === 0 ? (
              <tr>
                <td className="px-4 py-8 text-sm text-gray-500 text-center" colSpan={7}>No target data</td>
              </tr>
            ) : (
              list.map((row, i) => (
