...
export default function QuotationPreviewTemplate2({ data, companyBranches, user, onClose }) {
  const [templateHtml, setTemplateHtml] = React.useState('')
  const [loading, setLoading] = React.useState(false)

  React.useEffect(() => {
    const templateKey = data?.template || data?.templateKey || 'template2'
    console.log('🎨 QuotationPreviewTemplate2 - Loading template with key:', templateKey)
    console.log('📋 QuotationPreviewTemplate2 - Full data:', data)
  ...
  const templateData = QuotationDataMapper.prepareContext(
    ...
    companyBranches,
    user,
    data.template || data.templateKey || 'template2'
  )
